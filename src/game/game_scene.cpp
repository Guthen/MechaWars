...
void GameScene::init()
{
	...
	camera.set_bounds( 0, 0, map->get_size().x * 8, map->get_size().y * 8 );
...
void GameScene::update( float dt )
{
	...
	{
		DEFERED(
			GameScene* scene = GameManager::create<GameScene>( std::chrono::system_clock::now().time_since_epoch().count() );
			GameManager::change_scene_to( scene );
		);
		//map->generate( std::chrono::system_clock::now().time_since_epoch().count() );
	}
